...
_APPEARANCE_RULES = (
    "- Translate each OCEAN trait into VISIBLE physical cues. "
    "Read each value: >= 0.7 = high, <= 0.3 = low, otherwise neutral (omit).\n"
    "- Trait -> physical mapping (high / low):\n"
    "  - Openness: high = unconventional, artistic styling, alert curious eyes; "
    "low = plain, conventional, standard grooming.\n"
    "  - Conscientiousness: high = upright composed posture, immaculate neat "
    "grooming, crisp well-fitted attire, defined jaw; low = slack posture, "
    "disheveled hair, rumpled ill-fitting clothes.\n"
    "  - Extraversion: high = energetic open expansive stance, animated bright "
    "expression, healthy vital build; low = closed reserved arms-in stance, "
    "subdued neutral expression.\n"
    "  - Agreeableness: high = relaxed approachable posture, soft facial "
    "contours and fuller cheeks, warm open eyes, gentle smile; low = stiff "
    "guarded posture, lean sharp features, hard-set cool expression.\n"
    "  - Neuroticism: high = tense braced or hunched posture, worried wary "
    "expression, furrowed brow, narrowed eyes; low = relaxed calm settled "
    "posture, even composed expression.\n"
    "- Favor posture, build, grooming, and clothing structure over subtle "
    "facial detail (3D models blur fine expression).\n"
...
def appearance(body: AppearanceReq, request: Request, x_anthropic_key: Optional[str] = Header(None)) -> dict:
    ...
    prompt = (
        ...
        "Read each numeric OCEAN value above and apply the trait -> physical "
        "mapping in the rules to that value.\n\n"
        "Rules:\n"
        f"{_APPEARANCE_RULES}\n\n"
